"""Test with Manus sandbox browser (CDP) which was working at 11:00."""
import time, uuid
import boto3
    ...
    with sync_playwright() as p:
        ...
        for i in range(30):
            ...
            if body and len(body) > 30 and 'forbidden' not in body.lower():
                print(f"Content after {i}s: {body[:100]}")
                break
        ...
        if state['onName']:
            ...
            if inp:
                ...
                if state['onOtp']:
                    print("[+] SUCCESS! Name page passed with CDP browser!")
                if state['onErr']:
                    print("[!] ERR-837 with CDP browser too")
