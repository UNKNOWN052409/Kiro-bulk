"""Test with minimal name - fixed with longer waits."""
...
def wait_for_state(page, timeout=15, poll=1):
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Wait for page to navigate and settle
        time.sleep(12.0)
        
        # Check state
        page_info = get_page_state(page)
        print(f"[*] After email: hash={page_info['hash']}")
        print(f"[*] Body snippet: {page_info['snippet'][:150]}")
        
        # Fill minimal name and click Continue
        if page_info['onName']:
            print("[*] Filling minimal name 'A'...")
            page.evaluate("""() => {
                const inputs = document.querySelectorAll('input');
                for (const inp of inputs) {
                    const vis = inp.offsetWidth > 0 && inp.offsetHeight > 0 && !inp.disabled;
            ...
            page_info2 = get_page_state(page)
            print(f"[*] After name: hash={page_info2['hash']}")
            print(f"[*] Body snippet: {page_info2['snippet'][:300]}")
        else:
        ...
        if page_info2.get('onOtp'):
            ...
            if otp:
                ...
                print(f"[*] After OTP: hash={page_info3['hash']}")
                print(f"[*] Body snippet: {page_info3['snippet'][:300]}")
