    ...
    for i in range(15):
        ...
        try:
            ...
            if 'email' in body.lower() and 'continue' in body.lower():
                print(f"  Sign-in page loaded!")
                break
        except:
            pass
    
    # Enter email and click the primary submit button
    print("  Entering test email...")
    try:
        # Use test_id for the primary button
        inputs = page.locator('input').all()
        visible = [inp for inp in inputs if inp.is_visible()]
        if visible:
            visible[0].fill('test456@havenhaus.in')
            time.sleep(1)
            # Click the primary submit button (first button of type submit)
