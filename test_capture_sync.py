"""Capture token from associate_token response using sync callbacks."""
...
def on_response(response):
    ...
    if 'associate_token' in url:
        try:
            body = response.json()
            captured_data['associate_token'] = body
            print(f"\n[+] *** associate_token captured! Keys: {list(body.keys())}")
        except Exception:
            try:
                body = response.text()
                captured_data['associate_token'] = body
                print(f"[+] associate_token text: {body[:200]}")
            except:
                pass
    elif '/session/device' in url:
        try:
            body = response.json()
            captured_data['session_device'] = body
            print(f"[+] session/device captured! Keys: {list(body.keys())}")
        except Exception:
            pass
...
# Print captured data
print(f"\n[+] Captured keys: {list(captured_data.keys())}")
