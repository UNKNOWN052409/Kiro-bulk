...
def get_proxy_fallback(country='us'):
    """Get proxy from onrender_pool.json (authenticated proxy pool)."""
    ...
    if not hasattr(get_proxy_fallback, '_pool'):
        ...
        # Load from onrender_pool.json
        pool_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'working_proxies.json')
        if os.path.exists(pool_file):
            try:
                with open(pool_file, 'r') as pf:
                    raw_pool = _json.load(pf)
                get_proxy_fallback._pool = [
                    {'server': p['server'], 'username': p.get('username',''), 'password': p.get('password','')}
                    for p in raw_pool
                ]
                get_proxy_fallback._pool_time = now
                sp(f"  [+] Loaded {len(get_proxy_fallback._pool)} proxies from onrender_pool.json")
            except Exception as e:
                sp(f"  [!] Failed to load onrender_pool.json: {e}")
    pool = get_proxy_fallback._pool
    pool_time = get_proxy_fallback._pool_time
    if not pool:
        sp(f"  [!] No proxies available")
        return None
    # Pick a proxy not yet used in this batch
    used_servers = get_proxy_fallback._used
    available = [p for p in pool if p['server'] not in used_servers]
    if not available:
        get_proxy_fallback._used.clear()
        available = pool
    proxy = random.choice(available)
    get_proxy_fallback._used.add(proxy['server'])
    return proxy
...
def create_account(page, domain, run_idx=0, step_results=None, mail_provider=None):
    ...
    # Add response interception to capture OIDC tokens
    _captured_oidc_tokens = []
    _url_history = []
    def _on_oidc_response(response):
        """Intercept OIDC token endpoint responses to capture access/refresh tokens."""
        url = response.url
        if 'token' in url.lower() and ('oidc' in url.lower() or 'amazonaws' in url.lower()):
            try:
                body = response.text()
                if body:
                    try:
                        ...
                        if 'accessToken' in data or 'refreshToken' in data:
                            _captured_oidc_tokens.append(data)
                            sp(f"  [OIDC] Token response captured! access={data.get('accessToken','')[:20]}... refresh={data.get('refreshToken','')[:20]}...")
                    except json.JSONDecodeError:
                        pass
            except Exception:
                pass
    try:
        page.on("response", _on_oidc_response)
        page.on("framenavigated", lambda frame: _url_history.append(frame.url[:150]))
        sp("  [+] OIDC response interception + URL tracking enabled")
    except Exception as e:
        sp(f"  [!] Response interception setup failed: {e}")
        ...
        if state == "signup-start":
            if step_results.get('fill_otp'):
                ...
                if not diag_body or len(diag_body.strip()) == 0:
                    ...
                    if ns != "signup-start":
                        state = ns
                        # If we transitioned to password page, skip loop detection
                        if ns == "password":
                            sp("  [+] OTP verified! On password page - normal flow")
                            step_results['force_password'] = True
                            # Don't raise exception - let the state machine handle password state
                            # Skip the rest of loop detection by setting diag_body to indicate password
                            diag_body = "create password"  # This will trigger the password check below
                    else:
                        diag_body = (page.evaluate("() => document.body?.innerText?.substring(0,500)||''") or "")
            ...
            if code:
                ...
                if otp_info['type'] == 'split':
                    ...
                    if visible:
                        ...
                        sp(f"  [+] OTP filled: {code}")
                        time.sleep(random.uniform(1.0,2.5))
                        # Only click specific submit buttons - NOT "Change" or navigation buttons
                        clicked = False
                        # Try Enter key first (many OTP forms auto-submit on Enter)
                        try:
                            page.keyboard.press('Enter')
                            time.sleep(2.0)
                            # Check if page advanced
                            new_state = detect_state(page)
                            if new_state not in ("verify-otp", None):
                                sp(f"  [+] OTP submitted via Enter key -> {new_state}")
                                clicked = True
                            else:
                                clicked = False
                        except Exception:
                        ...
                        if not clicked:
                            clicked = (click_text(page, "Continue", timeout=3000) or \
                                ...
                                click_text(page, "Verify code", timeout=2000) or \
                        ...
                        # If no button clicked, try pressing Enter (many OTP forms auto-submit)
                        if not clicked:
                            page.keyboard.press('Enter')
                            sp("  [+] OTP submitted (Enter key)")
                        else:
                            sp(f"  [+] OTP submitted")
                    else:
            ...
            if pw_filled:
                ...
                # Dump URL history to see the redirect chain
                if _url_history:
                    sp(f"  [*] URL history ({len(_url_history)} entries):")
                    for idx, u in enumerate(_url_history[-15:]):
                        sp(f"    [{idx}] {u}")
...
def run_one(panel_url, panel_pass, domain, proxy_country, headless, config, db, no_proxy=False, mail_provider=None, extra_panels=None):
    ...
    # Clear profile before each run to avoid persistent sessions
    try:
    ...
    try:
        ...
        # Profile already cleared at start of run_one
