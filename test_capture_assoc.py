"""Capture token from associate_token response."""
...
async def on_response(response):
    ...
    if 'associate_token' in url:
        global assoc_response
        try:
            assoc_response = await response.json()
            print(f"\n[+] *** associate_token response: ***")
            print(json.dumps(assoc_response, indent=2)[:2000])
        except Exception as e:
            body = await response.text()
            print(f"[+] associate_token raw: {body[:2000]}")
            assoc_response = body
    elif '/session/device' in url and 'deviceContext' not in url:
        global device_response
        try:
            device_response = await response.json()
            print(f"\n[+] *** session/device response: ***")
            print(json.dumps(device_response, indent=2)[:2000])
        except Exception as e:
            pass
...
if assoc_response:
    ...
    if isinstance(assoc_response, dict):
        print(json.dumps(assoc_response, indent=2)[:3000])
    else:
        print(str(assoc_response)[:3000])
else:
    print("\n[!] No associate_token response captured")

if device_response:
    print("\n[+] === SESSION DEVICE RESPONSE ===")
    print(json.dumps(device_response, indent=2)[:3000])

# Also try to extract from the final URL's deviceContextId
print("\n[+] Done")
