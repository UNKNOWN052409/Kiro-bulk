"""Ultra-aggressive poll: start polling BEFORE clicking Allow."""
...
def poll_for_token():
    global captured_token, poll_start_time
    poll_start_time = time.time()
    deadline = poll_start_time + expires_in
    attempts = 0
    while time.time() < deadline:
        attempts += 1
        # No sleep on first attempt, then 10ms
        if attempts > 1:
            time.sleep(0.01)
        try:
            ...
            if token:
                ...
                print(f"\n[+] *** TOKEN CAPTURED after {attempts} attempts ({time.time()-poll_start_time:.2f}s)! ***")
                poll_done.set()
                return
            ...
            if err not in ('AuthorizationPendingException', 'SlowDownException'):
                ...
                print(f"\n[!] Poll error after {attempts} attempts ({elapsed:.2f}s): {err}")
                if 'already redeemed' in str(e):
                    print(f"    Message: {e.response['Error'].get('Message', '')}")
                poll_done.set()
                return
        except Exception as e:
            elapsed = time.time() - poll_start_time
            print(f"\n[!] Exception after {attempts} attempts ({elapsed:.2f}s): {e}")
            poll_done.set()
            return
    print(f"\n[!] Poll timeout after {attempts} attempts")
    poll_done.set()
...
if captured_token:
    ...
    print(f"[+] Refresh Token: {captured_token[:50]}...")
else:
    print("\n[!] Token not captured")
