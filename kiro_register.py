...
async def register(headless=True, auto_login=True, skip_onboard=True,
    ...
    try:
        ...
        # Camoufox launch kwargs (mirrors Playwright chromium)
        launch_kwargs = {"headless": headless, "args": launch_args, "os": "windows"}
        async with AsyncCamoufox(geoip=False, humanize=True, **launch_kwargs) as browser:
            # Minimal context: Camoufox handles fingerprinting natively.
            # Avoid add_init_script / extra kwargs that hang new_page() in Camoufox.
            ...
            # Skip heavy fingerprint init-script (caused new_page() to hang in Camoufox).
            # Camoufox already randomizes the fingerprint.
