"""Use browser fetch to call create_token after Allow click."""
...
with sync_playwright() as p:
    ...
    # NOW: Use the browser to call create_token via fetch
    # The OIDC endpoint is on a different domain (oidc.us-east-1.amazonaws.com)
    # The browser is on view.awsapps.com - CORS might block this
    # But let's try
    print("\n[*] Calling create_token from browser...")
    result = page.evaluate(f"""async () => {{
        try {{
            const resp = await fetch('https://oidc.us-east-1.amazonaws.com/token', {{
                method: 'POST',
    ...
    print(f"[+] create_token from browser: {json.dumps(result, indent=2)[:500]}")
    
    # Also try the token endpoint without the /token path
    result2 = page.evaluate(f"""async () => {{
        try {{
            const resp = await fetch('https://oidc.us-east-1.amazonaws.com/', {{
                method: 'POST',
                headers: {{'Content-Type': 'application/x-www-form-urlencoded'}},
                body: new URLSearchParams({{
                    client_id: '{reg["clientId"]}',
    ...
    print(f"[+] create_token from browser (root): {json.dumps(result2, indent=2)[:500]}")
